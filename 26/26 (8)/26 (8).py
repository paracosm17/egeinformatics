...
if remain > 0:
    for B in Bs:                        # Проходимся по партиям B
        if remain - (B[0]*B[1]) > 0:    # Если купить всю партию и остануться деньги, то продолжаем покупать
            remain -= B[0]*B[1]
            b += B[1]                   # Считаем товары 
            continue
        else:                           # Если денег на целую партию уже не хватает
            for _ in range(1, B[1]):    # То проходимся циклом по этой партии
                if remain - B[0] > 0:   # Если нам хватает денег купить один товар
                    remain -= B[0]      # То покупаем
                    b += 1              # Считаем товары
                else:
                    break
            break
